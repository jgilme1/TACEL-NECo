...
public class NECoStanfordAnnotatorHelperMethods {
	...
	private Map<String,Annotation> corefAnnotationMap= new HashMap<String,Annotation>();
	private Map<String,Annotation> neAnnotationMap = new HashMap<String,Annotation>();
	
	
	public void clearAnnotations(){
		corefAnnotationMap.clear();
		neAnnotationMap.clear();
	}
	
	public NECoStanfordAnnotatorHelperMethods(Boolean usePipeline){	
		if(usePipeline){
	...
	private List<CorefMention> getCorefMentions(String docName, Annotation document, Integer begOffset, Integer endOffset){
		if(!corefAnnotationMap.containsKey(docName)){		
			...
			corefAnnotationMap.put(docName, document);
			}
			catch(Exception e){
			return null;
			}
			...
			document = corefAnnotationMap.get(docName);
		}
	...
	public List<Interval> getCorefIntervals(String docName, String xmlString, Integer begOffset, Integer endOffset) {
		    
		    Annotation document = new Annotation(xmlString);
			List<CorefMention> listOfCorefMentions = getCorefMentions(docName, document,begOffset,endOffset);
			if(corefAnnotationMap.containsKey(docName)){
				document = corefAnnotationMap.get(docName);
			}
	...
	public String getCorefRepresentativeString(String docName, String xmlString, Integer begOffset, Integer endOffset) {
	    Annotation document = new Annotation(xmlString);
		List<CorefMention> listOfCorefMentions = getCorefMentions(docName,document,begOffset,endOffset);
		if(listOfCorefMentions == null){
		  return null;
	...
	public List<String> getCorefStringMentions(String docName, String xmlString, Integer begOffset, Integer endOffset) {
	    Annotation document = new Annotation(xmlString);
		List<CorefMention> listOfCorefMentions = getCorefMentions(docName,document,begOffset,endOffset);
		if(listOfCorefMentions == null){
	...
	private List<List<CoreLabel>> getNamedEntityTokens(String docName, Annotation document){
		if(!neAnnotationMap.containsKey(docName)){		
			scala.actors.threadpool.ExecutorService executor = Executors.newSingleThreadExecutor();
			try{
			executor.submit(new AnnotationRunnable(document,corefPipeline)).get(6, TimeUnit.MINUTES);
			neAnnotationMap.put(docName, document);
			}
			catch(Exception e){
			return null;
			}
			...
			document = neAnnotationMap.get(docName);
		}
		List<CoreMap> sentences = document.get(SentencesAnnotation.class);
	...
	public List<String> getMatchingNamedEntities(String docName, String xmlString, Integer begOffset, Integer endOffset) {
		Annotation document = new Annotation(xmlString);
		List<List<CoreLabel>> namedEntityTokens = getNamedEntityTokens(docName,document);
	...
	public List<String> getNamedEntitiesByType(String docName, String namedEntityType, String xmlString){
		Annotation document = new Annotation(xmlString);
		List<List<CoreLabel>> namedEntityTokens = getNamedEntityTokens(docName, document);
		if(namedEntityTokens == null){
			return new ArrayList<String>();
	...
	public List<String> getNamedEntities(String docName, String str){
